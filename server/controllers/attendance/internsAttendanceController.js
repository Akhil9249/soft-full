...
const getMentorInternsFromWeeklySchedule = async (mentorId) => {
  try {
    ...
    if (!weeklySchedules || weeklySchedules.length === 0) {
      return { interns: [], message: "No weekly schedules found for this mentor" };
    }

    // Extract all unique interns from all batches in all schedule documents
    const allInterns = [];
        ...
        scheduleDoc.schedule.sub_details.batch.forEach(batch => {
          if (batch && batch.interns && batch.interns.length > 0) {
            batch.interns.forEach(intern => {
              if (intern && !internIds.has(intern._id.toString())) {
...
const addInternsAttendance = async (req, res) => {
  try {
    ...
    if (!intern || !date || typeof status !== 'boolean' || !markedBy) {
      return res.status(400).json({
        message: "Intern, date, status, and markedBy are required"
      });
    }

    // Default mentor to markedBy if not explicitly provided
    const mentor = req.body.mentor || markedBy;
    ...
    // Check for duplicate attendance on the same date by the same mentor for the same batch
    const searchParams = {
      intern: intern,
      date: formattedDate,
      mentor: mentor
    ...
    if (batchId) {
      searchParams.batch = batchId;
    }
    const existingAttendance = await InternsAttendance.findOne(searchParams);

    if (existingAttendance) {
      return res.status(400).json({
        message: "Attendance already marked for this intern on this date by this mentor for this batch"
      });
    }
    ...
    const newAttendance = await InternsAttendance.create({
      ...
      status,
      mentor, // newly required schema constraint
      checkInTime,
      checkOutTime,
      totalHours,
      remarks,
    ...
    // Populate the response with intern, markedBy, and batch details
...
const getInternsAttendance = async (req, res) => {
  try {
    ...
    if (userRole === 'mentor') {
      ...
      filter.mentor = userId; // Isolate attendance records by mentor
    }
    ...
    if (userRole === 'super admin' || userRole === 'admin') {
      ...
      res.status(200).json({
        message: allowedInternIds
          ...
          : "Daily attendance records created successfully for all ongoing interns"
      });
    } else if (userRole === 'mentor') {
...
const updateSingleInternAttendance = async (req, res) => {
  try {
    const { internId, date, status, remarks, batch } = req.body;
    ...
    const formattedDate = `${year}-${month}-${day}`;
    // Pass batch as well
    const attendance = await updateInternAttendance(internId, formattedDate, status, markedBy, remarks, batch);
...
const getAttendanceSummaryReport = async (req, res) => {
  try {
    ...
    const summary = await getAttendanceSummary(formattedStartDate, formattedEndDate, allowedInternIds, userRole === 'mentor' ? userId : null);
...
const getInternsByWeeklyScheduleFilters = async (filters = {}) => {
  try {
    ...
    // If days is provided, filter by days (schema uses 'day')
    if (days) {
      weeklyScheduleQuery['schedule.sub_details.day'] = days;
    }
    ...
    weeklySchedules.forEach(scheduleDoc => {
      if (!scheduleDoc.schedule || !scheduleDoc.schedule.sub_details) return;

      const timeSlot = scheduleDoc.schedule;
      const subDetail = timeSlot.sub_details;

      if (subDetail.batch && Array.isArray(subDetail.batch)) {
        subDetail.batch.forEach(batch => {
          if (batch && batch.interns && batch.interns.length > 0) {
            batch.interns.forEach(intern => {
              ...
              // Filter by days if days is provided (schema uses 'day')
              const scheduleDayId = subDetail.day ? (subDetail.day._id ? subDetail.day._id.toString() : subDetail.day.toString()) : null;
              if (days && scheduleDayId !== days) {
                shouldInclude = false;
              }

              // Filter by timing if timingId is provided
              const scheduleTimeId = timeSlot.time ? (timeSlot.time._id ? timeSlot.time._id.toString() : timeSlot.time.toString()) : null;
              if (timingId && scheduleTimeId !== timingId) {
                shouldInclude = false;
              }

              if (shouldInclude && intern && !internIds.has(intern._id.toString())) {
                internIds.add(intern._id.toString());
                allInterns.push(intern);
              }
            });
...
const getInternsByAttendanceDate = async (req, res) => {
  try {
    ...
    if (userRole === 'mentor') {
      ...
      query.mentor = userId; // Ensure mentor only sees their own attendance records
    }
    // For Super Admin and Admin, no additional filtering is applied - they can see all interns
    ...
    // 1) Find all interns who should be displayed based on the applied filters/mentor status
    const internQuery = {
      $or: [
        { courseStatus: "Active" },
    ...
    const allApplicableInterns = await Intern.find(internQuery).populate([
    ...
    // 2) Get attendance records for the specified date
    const attendanceRecords = await InternsAttendance.find(query)
      .populate('markedBy', 'fullName email')
      .sort({ createdAt: -1 });

    // Create a dictionary of existing attendance records for O(1) loop lookup
    const attendanceMap = {};
    attendanceRecords.forEach(record => {
      if (record.intern) {
        attendanceMap[record.intern.toString()] = record;
      }
    });

    // 3) Map existing attendance onto the list of interns
    const internsWithAndWithoutAttendance = allApplicableInterns.map(intern => {
      const record = attendanceMap[intern._id.toString()];
      return {
        _id: intern._id,
        fullName: intern.fullName,
        email: intern.email,
        role: intern.role,
        courseStatus: intern.courseStatus,
        branchId: intern.branch?._id,
        branchName: intern.branch?.branchName,
        courseId: intern.course?._id,
        courseName: intern.course?.courseName,
        categoryId: intern.course?.category?._id,
        categoryName: intern.course?.category?.categoryName,
        ...
        remarks: record ? record.remarks : "Attendance not yet created",
    ...
    res.status(200).json({
      ...
      data: internsWithAndWithoutAttendance,
      totalCount: internsWithAndWithoutAttendance.length
    });
  } catch (error) {
    res.status(500).json({
...
const getInternsAttendanceByMonth = async (req, res) => {
  try {
    const { month, year, branchId, mentorId } = req.query;
    const userId = req.userId;
    ...
    console.log("month", month, "year", year, "branchId", branchId, "mentorId", mentorId);
    ...
    if (branchId) {
      ...
      const weeklyScheduleResult = await getInternsByWeeklyScheduleFilters({
        branchId,
        mentorId: userRole === 'mentor' ? userId : (mentorId || null)
      });
    ...
    const internMentorMap = new Map();

    // Initialize all (intern, mentor) combinations with empty attendance array
    attendanceRecords.forEach(record => {
      const internId = record.intern._id.toString();
      const mentorId = record.markedBy ? record.markedBy._id.toString() : 'unknown';
      const key = `${internId}_${mentorId}`;

      if (!internMentorMap.has(key)) {
        internMentorMap.set(key, {
          ...
          mentorId: mentorId,
          mentorName: record.markedBy ? record.markedBy.fullName : 'Unknown Mentor',
    ...
    attendanceRecords.forEach(record => {
      ...
      const mentorId = record.markedBy ? record.markedBy._id.toString() : 'unknown';
      const key = `${internId}_${mentorId}`;
      const entry = internMentorMap.get(key);
    ...
    const monthData = Array.from(internMentorMap.values());

    // Calculate summary statistics
    const summary = {
      totalInterns: new Set(monthData.map(d => d._id.toString())).size,
