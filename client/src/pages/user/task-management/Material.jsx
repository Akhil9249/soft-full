...
const Material = () => {
  ...
  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= pagination.totalPages) {
      ...
      loadMaterials(newPage, searchTerm, filters.mentor);
    }
  };
  ...
  // Load initial data
  useEffect(() => {
    loadMaterials(pagination.currentPage, searchTerm, filters.mentor);
  }, []);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      loadMaterials(1, searchTerm, filters.mentor);
    }, 500); // 500ms debounce

    return () => clearTimeout(timeoutId);
  }, [searchTerm, filters.mentor]);

  // Load materials
  const loadMaterials = async (page = 1, search = '', mentor = '') => {
    try {
      setLoading(true);
      ...
      const response = await userService.getMyMaterialsData(queryParams.toString());
  ...
  const handleDownloadAttachment = async (material) => {
    try {
      ...
      // Fetch file as blob using UserService method (ensures proper authentication)
      const response = await userService.downloadMaterialAttachment(materialId);
  ...
  return (
    <>
      ...
      <div className='min-h-screen bg-gray-50/50 p-6 font-sans text-gray-800'>
        <Navbar headData="Material" activeTab="Material List">
              ...
              {loading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 mb-4">
                    {searchTerm || filters.audience || filters.mentor ? 'Try adjusting your search or filter criteria.' : 'No materials have been shared with you yet.'}
                  </p>
                </div>
              ) : (
                <>
